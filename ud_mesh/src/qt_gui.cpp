...
Graphical_UI::Graphical_UI() : filename("quicksave.pcd"), glviewer(NULL)
{
    ...
    mouseHelpText = new QString(tr(
                "<p><b>UD 3D mesh making tool based on GPU_SURF</b>"
                ...
                "    <li><i>Double click on background:</i> reset view to camera pose.</li><ul></p>"
                 */
                 "<p><b>Based on the RGBDSLAM code</b></p>"
 ...
 feature_flow_image_label = new QLabel(*mouseHelpText);
    ...
    QString message = tr("Ready for 3D scanning");
    ...
    infoLabel = new QLabel(tr("<i>Waiting for the user to start (space or enter)</i>"));
    infoLabel->setFrameStyle(QFrame::StyledPanel | QFrame::Sunken);
    infoLabel->setAlignment(Qt::AlignRight);
    statusBar()->addPermanentWidget(infoLabel, 0);

    setWindowTitle(tr("UD_MESH"));
...
void Graphical_UI::about() {
    QMessageBox::about(this, tr("About UD_MESH"), *infoText + *licenseText);
}
